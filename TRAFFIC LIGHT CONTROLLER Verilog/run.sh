...
vlib work && vlog '-timescale' '1ns/1ps' design.sv testbench.sv  && vsim -c -do "vsim +access+r; run -all; exit"  ; echo 'Creating result.zip...' && zip -r /tmp/tmp_zip_file_123play.zip . && mv /tmp/tmp_zip_file_123play.zip result.zip